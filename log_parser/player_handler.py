...
from config.settings import PLAYERS_DATA_FILE
...
class PlayerHandler:
    ...
    def _handle_login(self, steam_id, player_name, log_file):
        ...
        if steam_id in self.players:
            logger.debug(f"Игрок с ID {steam_id} уже онлайн. Пропускаем.")
            return False
        ...
        if not player_name:
            if steam_id in self.players_in_file and self.players_in_file[steam_id]["name"]:
                ...
                logger.debug(f"Ник для игрока {steam_id} взят из файла: {player_name}.")
            else:
        ...
        if steam_id not in self.players:
            ...
            logger.info(f"Игрок {player_name} ({steam_id}) подключился. Количесство игроков: {len(self.players)}")

        if steam_id not in self.players_in_file:
            self.players_in_file[steam_id] = {"name": player_name, "score": 0, "duration": 0.0}
            logger.debug(f"Игрок {player_name} ({steam_id}) добавлен в файл.")
        elif self.players_in_file[steam_id]["name"] != player_name:
            self.players_in_file[steam_id]["name"] = player_name
            logger.debug(f"Ник игрока {steam_id} обновлен на {player_name}.")
    ...
    def _handle_logout(self, steam_id, log_file):
        ...
        if self.player_log_files.get(steam_id) != log_file:
            logger.debug(f"Игрок с ID {steam_id} вышел из другого файла. Пропускаем.")
            return False
        ...
        logger.info(f"Игрок {player_name} ({steam_id}) отключился. Количесство игроков: {len(self.players)}")
